---
obsidianUIMode: preview
NoteIcon: class
NoteStatus: Active
type: class
status: Ativa
rules_status: Oficial / consulta externa
campaign_status: Ativa
visibility: Jogadores
spoiler_level: none
gm_secret: false
created_by: Sage
name: Ladrão
aliases:
  - Ladino
system: Old Dragon 2E
class_group: Especialista
primary_attribute: Destreza
level:
danger_level: Baixo
thumbnail: zz_media/thumbnails/th_ladrao.png
cover: zz_media/class/ladrao.png
chapters: []
tags:
  - classe
  - ladrao
  - old-dragon
  - class
---

# Ladrão

> [!NOTE|clean no-i right]+ Retrato
> ![[zz_media/class/ladrao.png|400]]

## Visão Geral

Ladrão é a classe de exploração, furtividade, arrombamento, percepção de risco, infiltração e ataques oportunistas.

Esta nota reúne a consulta mecânica usada em mesa: dados de vida, restrições de equipamento, progressão, talentos e números principais.

## Papel no Grupo

- Explorar lugares perigosos.
- Lidar com armadilhas, fechaduras e infiltração.
- Obter vantagem por astúcia, posicionamento e oportunismo.
- Resolver problemas sem confronto direto.

## Papel no Mundo

Ladrões podem ser criminosos, espiões, batedores, caçadores de tesouro, sobreviventes urbanos, contrabandistas ou agentes de guildas.

## Ladrão em Jogo

| Elemento | Regra de consulta |
|---|---|
| Dados de Vida | D6 |
| Armaduras | Apenas couro; não pode usar escudos se quiser usar talentos |
| Armas | Armas de ataque à distância e armas pequenas de uma mão |
| Armas maiores | Penalidade de -2 nos ataques e no dano |
| Itens mágicos | Não usa varinhas, bastões e pergaminhos |
| Talentos | Arrombar, Armadilhas, Escalar, Furtividade, Punga, Percepção e Ataque furtivo |
| Guilda e seguidores | A partir do 11º nível, pode organizar guilda e atrair seguidores |

## Progressão

| Nível | XP | DV/PV | BA | JP |
|---:|---:|---:|---:|---:|
| 1 | 0 | 1 | +1 | 15 |
| 2 | 1.250 | 2 | +1 | 15 |
| 3 | 2.500 | 3 | +2 | 15 |
| 4 | 5.000 | 4 | +2 | 14 |
| 5 | 10.000 | 5 | +2 | 14 |
| 6 | 20.000 | 6 | +3 | 14 |
| 7 | 40.000 | 7 | +3 | 13 |
| 8 | 80.000 | 8 | +3 | 13 |
| 9 | 160.000 | 9 | +4 | 13 |
| 10 | 240.000 | +1 PV | +4 | 12 |
| 11 | 400.000 | +1 PV | +4 | 12 |
| 12 | 520.000 | +2 PV | +5 | 12 |
| 13 | 640.000 | +2 PV | +5 | 11 |
| 14 | 760.000 | +2 PV | +5 | 11 |
| 15 | 880.000 | +3 PV | +6 | 11 |
| 16 | 1.000.000 | +3 PV | +6 | 10 |
| 17 | 1.120.000 | +3 PV | +6 | 10 |
| 18 | 1.240.000 | +4 PV | +7 | 10 |
| 19 | 1.360.000 | +4 PV | +7 | 9 |
| 20 | 1.480.000 | +4 PV | +7 | 9 |

## Talentos de Ladrão

`dx` indica o dado de tempo para arrombar/desarmar. Percepção usa 1d6.

| Nível | Arrombar | Armadilhas | Escalar | Furtividade | Punga | Percepção | Ataque furtivo |
|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 15%/1d8 | 10%/1d8 | 80% | 20% | 20% | 1-2 | x2 |
| 2 | 20%/1d8 | 15%/1d8 | 81% | 25% | 25% | 1-2 | x2 |
| 3 | 25%/1d8 | 20%/1d8 | 82% | 30% | 30% | 1-2 | x2 |
| 4 | 30%/1d8 | 25%/1d8 | 83% | 35% | 35% | 1-2 | x2 |
| 5 | 35%/1d8 | 30%/1d8 | 84% | 40% | 40% | 1-3 | x2 |
| 6 | 40%/1d6 | 35%/1d6 | 85% | 45% | 45% | 1-3 | x3 |
| 7 | 45%/1d6 | 40%/1d6 | 86% | 50% | 50% | 1-3 | x3 |
| 8 | 50%/1d6 | 45%/1d6 | 87% | 55% | 55% | 1-3 | x3 |
| 9 | 55%/1d6 | 50%/1d6 | 88% | 60% | 60% | 1-3 | x3 |
| 10 | 60%/1d6 | 55%/1d6 | 89% | 65% | 65% | 1-4 | x3 |
| 11 | 62%/1d4 | 60%/1d4 | 90% | 70% | 70% | 1-4 | x3 |
| 12 | 64%/1d4 | 62%/1d4 | 91% | 72% | 72% | 1-4 | x4 |
| 13 | 66%/1d4 | 64%/1d4 | 92% | 74% | 74% | 1-4 | x4 |
| 14 | 68%/1d4 | 66%/1d4 | 93% | 76% | 76% | 1-4 | x4 |
| 15 | 70%/1d4 | 68%/1d4 | 94% | 78% | 78% | 1-4 | x4 |
| 16 | 72%/1 | 70%/1 | 95% | 80% | 80% | 1-5 | x4 |
| 17 | 74%/1 | 72%/1 | 96% | 82% | 82% | 1-5 | x4 |
| 18 | 76%/1 | 74%/1 | 97% | 84% | 84% | 1-5 | x5 |
| 19 | 78%/1 | 76%/1 | 98% | 86% | 86% | 1-5 | x5 |
| 20 | 80%/1 | 78%/1 | 99% | 88% | 88% | 1-5 | x5 |

## Especializações

| Caminho | Nível | Benefício de consulta |
|---|---:|---|
| Ranger | 5 | Desloca-se sem deixar rastro; escolhe criatura protegida/caçada; recebe +1 em jogadas e CA envolvendo essa criatura para cada 5 níveis; pode usar reação de Carisma contra animais |
| Ranger | 8 | Pode lutar com duas armas com -2 na segunda; amplia uso de Escalar e Percepção; não é surpreendido e sempre surpreende inimigo que não seja outro ranger |

## Observações de Personagem

- [[Varkh Nimalis]] possui identidade narrativa próxima de ladrão, mas sua classe operacional atual é [[Alquimista]].
- [[Raziel]] não usa mais Ladrão como classe operacional; sua classe atual é [[Hemomante]].

## Personagens Relacionados

```dataview
TABLE thumbnail, race, status, location, faction
FROM "Characters"
WHERE class = "Ladrão" OR class = "Ladrao" OR class = this.file.link
SORT file.name ASC
```

## Uso em Mesa

- Como apresentar: mão rápida, ouvido atento, sombra, tranca aberta ou plano improvisado.
- O que os jogadores sabem: Ladrões resolvem riscos de exploração e infiltração.
- O que manter em aberto no [[ESTADO_DA_CAMPANHA]]: vínculos criminais e contatos ocultos.
- Como entra em cena: dungeons, cidades, infiltração, fuga, golpe e investigação.
- Ganchos: guildas, dívida, segredo, item roubado, máscara social.

## Pendências do Sage

- Definir se algum personagem atual usará Ladrão mecanicamente.
- Separar o que é identidade narrativa de Varkh da sua classe Alquimista.
