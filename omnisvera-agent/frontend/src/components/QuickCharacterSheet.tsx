...
import { listPlayableCharacters, mediaUrlFromVaultPath, PlayableCharacterSummary } from "../api";
...
export default function QuickCharacterSheet({ hidden, onOpen }: { hidden?: boolean; onOpen: () => void }) {
  ...
  return <aside className={`quick-character-sheet ${open ? "open" : ""}`} aria-label="Ficha rápida">
    {!open ? <button className="quick-sheet-trigger" onClick={() => setOpen(true)} aria-label={`Abrir ficha rápida de ${character.name}`}>
      ...
      {characters.length > 1 && <select aria-label="Personagem da ficha rápida" value={character.id} onChange={(event) => { setSelectedId(event.target.value); localStorage.setItem("omnisvera_selected_character", event.target.value); }}>
