...
export default function ScenePanel({ mode }: { mode: "gm" | "player" }) {
  ...
  return <section className="panel scene-page">
    <header className="scene-hero"><div><p className="eyebrow">Cena atual · {statusLabel(scene.status)}</p><h2>{scene.title}</h2><p className="scene-location">⌖ {scene.location_name}</p><p>{scene.public_description}</p><button className="secondary-button" onClick={() => window.dispatchEvent(new CustomEvent("omnisvera-open-map"))}>Abrir mapa e viagens</button></div><div className="scene-objective"><small>Objetivo</small><strong>{scene.objective || "Não informado"}</strong></div></header>
