---
obsidianUIMode: preview
NoteIcon: location
NoteStatus: Active
type: location
status: Ruínas
campaign_status: Ativo
visibility: Jogadores
spoiler_level: light
gm_secret: false
territory: "[[EARTHROPO/EARTHROPO|Earthropo]]"
location:
region: Sudeste de [[Nimalia]]
thumbnail: zz_media/thumbnails/ruinas_de_valthor.png
cover: zz_media/thumbnails/ruinas_de_valthor.png
info: Ruínas de um antigo reino próspero ao sudeste de Nimalia.
description: Ruínas antigas ao sudeste de Nimalia, ligadas a histórias esquecidas e ao passado profundo de Earthropo.
subtype: ruin
work_status: Em desenvolvimento
canon_status: Working Canon
created_by: Sage
requires_review: true
tags:
  - location
  - local
  - ruinas
  - valthor
  - earthropo
  - raziel
---

# Ruínas de Valthor

> [!NOTE|clean no-i right]+ Ruínas de Valthor
> ![[zz_media/thumbnails/ruinas_de_valthor.png|400]]

> [!world]- SINOPSE PÚBLICA
> Valthor foi um reino antigo e próspero. Hoje restam ruínas ao sudeste de [[Nimalia]], associadas a histórias esquecidas, cavernas profundas e ecos do passado de Earthropo.

## Visão Geral

Ruínas localizadas a sudeste de [[Nimalia]]. Valthor foi um reino antigo e próspero antes de sua queda.

A civilização que construiu Valthor, sua relação com os [[Campos de Earthropo]] e os motivos de sua queda ainda precisam ser definidos. A região deve funcionar como ponto de mistério antigo, exploração e descoberta gradual.

## Função no Mapa

Valthor possui marcador aproximado no [[MAPA DE EARTHROPO]] e no [[MAPA DE NIMALIA]], sobre um complexo em ruínas no sudeste. O ponto confirma apenas a região geral; não define fronteiras, estradas ou domínio político.

## Relações

- [[Nimalia]] — fica ao sudeste do reino.
- [[Campos de Earthropo]] — relação histórica ainda em revisão.
- [[Raziel]] — a história do personagem possui ligação com ruínas e estruturas antigas da região, mas detalhes devem ser revelados em jogo.
- [[Clã Sanguinallis]] — possível relação antiga a ser consolidada conforme a lore de Raziel.
- [[Ancião Primordial]] — ligado às cavernas abissais sob as ruínas na lore de Raziel.
- [[Sangue Antigo]] — relação de bastidor com a transformação de Raziel.

## Segredos do Mestre

- Não explicar cedo demais o que havia sob Valthor.
- Não revelar automaticamente a ligação entre Valthor, Raziel, Ancião Primordial e Sangue Antigo.
- Tratar Valthor como ruína explorável antes de tratá-la como resposta cosmológica.

## Uso em Mesa

- Como apresentar: ruínas antigas, parcialmente esquecidas, marcadas por silêncio, pedra quebrada e sinais de civilização perdida.
- O que os jogadores sabem: Valthor foi um reino antigo e hoje é um conjunto de ruínas ao sudeste de Nimalia.
- O que apenas o mestre sabe: sua relação com Raziel, Ancião Primordial e Sangue Antigo ainda deve ser dosada.
- Como entra em cena: exploração, rumor, mapa incompleto, busca por relíquias ou pistas ligadas ao passado de Earthropo.
- Ganchos: cavernas, inscrições antigas, facções buscando relíquias e ecos de guerras antigas.

## Pendências do Sage

- Definir fronteiras e escala do antigo reino de Valthor.
- Decidir quais detalhes da origem de Raziel podem aparecer publicamente.
- Definir se Valthor terá mapa próprio, dungeon ou apenas marcador regional.
