---
obsidianUIMode: preview
NoteIcon: faction
NoteStatus: Active
type: faction
visibility: Jogadores
spoiler_level: light
gm_secret: false
status: Ativa
campaign_status: Ativa
leader: "[[Augustus Terra Decimus]]"
location: "[[Nimalis]]"
territory: "[[Nimalia|Reino de Nimalia]]"
thumbnail: zz_media/thumbnails/th_coroa_de_nimalia.PNG
cover: zz_media/faction/coroa_de_nimalia.png
info: Autoridade monárquica do Reino de Nimalia.
description: Poder político central de Nimalia, liderado por Augustus Terra Decimus e apoiado por nobreza, guarda e burocracia real.
chapters: []
subtype: noble_house
work_status: Em desenvolvimento
canon_status: Working Canon
created_by: Sage
requires_review: true
tags:
  - faction
  - faccao
  - coroa-de-nimalia
  - monarquia
  - nobreza
  - nimalia
---

# Coroa de Nimalia

> [!NOTE|clean no-i right]+ Retrato
> ![[zz_media/faction/coroa_de_nimalia.png|400]]

> [!world]- SINOPSE PÚBLICA
> A Coroa de Nimalia representa a autoridade formal do reino dos antropos: leis, impostos, fronteiras, exército, diplomacia e estabilidade sob o rei soberano [[Augustus Terra Decimus]].

## Visão Geral

A nobreza e monarquia do [[Nimalia|Reino de Nimalia]], o principal reino dos antropos em Earthropo. Controlam as terras e o exército do reino.

## Liderança e Estrutura

- **Rei soberano:** [[Augustus Terra Decimus]]
- **Corte Real:** Nobres e conselheiros em [[Nimalis]]
- **Força militar ligada:** [[Guarda Real de Nimalia]]
- **Classe política ligada:** [[Nobreza de Nimalia]]

## Função no Mundo

A Coroa representa a autoridade formal de Nimalia. Ela mantém leis, impostos, fronteiras, rotas, exército e diplomacia do reino.

## Relações de Poder

| braço | função |
|---|---|
| [[Augustus Terra Decimus]] | Rei soberano. |
| [[Nobreza de Nimalia]] | Base social, política e disputas de influência. |
| [[Guarda Real de Nimalia]] | Braço militar e ordem pública. |
| [[Casa da Moeda de Nimalia]] | Controle monetário e possíveis investigações econômicas. |
| [[Guilda dos Mercadores]] | Parceira econômica e possível fonte de tensão. |

## Objetivos Públicos

- Manter a estabilidade do reino.
- Proteger as rotas e fronteiras de Nimalia.
- Sustentar a autoridade de [[Augustus Terra Decimus]].
- Administrar relações com guildas, nobres, assentamentos e forças militares.

## Relações

- [[Nimalia]] — reino governado pela Coroa.
- [[Nimalis]] — capital e sede do poder real.
- [[Guarda Real de Nimalia]] — braço militar oficial.
- [[Nobreza de Nimalia]] — base política e social do regime.
- [[Guilda dos Mercadores]] — parceira e possível fonte de tensão econômica.
- [[Conclave dos Errantes]] — grupo útil, mas difícil de controlar totalmente.

## Personagens Relacionados

```dataview
TABLE thumbnail, location, status, role
FROM "Characters"
WHERE contains(lower(string(faction)), "coroa de nimalia")
OR contains(tags, "coroa-de-nimalia")
SORT file.name ASC
```

## Uso em Mesa

- Como apresentar: autoridade legítima, burocrática e poderosa do reino.
- O que os jogadores sabem: a Coroa governa Nimalia a partir de [[Nimalis]].
- Como entra em cena: patrulhas, decretos, contratos, guardas, impostos, convocações e rumores de corte.
- Ganchos: remédios falsos, fronteiras inseguras, tensão com guildas, nobres ambiciosos e segredos antigos que a Coroa talvez queira controlar.

## Pendências do Sage

- Definir principais casas nobres.
- Definir conselheiros próximos de Augustus.
- Definir quanto a Coroa sabe sobre o [[Véu Cinzento]] e estruturas antigas.
