"""Synthetic Brain Integration — isolated capstone, not an external AI handoff.
...
def run_capstone(root, clock):
    ...
    _store_ns["utc_now"] = lambda: clock.now.return_value.isoformat()
...
class CapstoneTests(unittest.TestCase):
    def test_synthetic_discovery_and_commit(self):
        # Provider fixture dates and commit share an explicit controlled clock.
        ...
        with tempfile.TemporaryDirectory() as directory, patch.dict(sys.modules):
            evidence = run_capstone(Path(directory), clock)
            self.assertTrue(evidence["validation"]["valid"])
            self.assertEqual(evidence["prediction"]["status"], "open")
            self.assertEqual(evidence["commit_result"]["status"], "created")
