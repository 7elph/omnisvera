# Índice de Entidades

Índice das entidades existentes no vault Omnisvera.

## Personagens

### Personagens jogadores
- Vezemir - O Bastardo de Ferro
- Varkh Nimalis - O Corvo da Maré Baixa
- Raziel - O Espectro da Névoa de Sangue

### NPCs individuais
- Augustus Terra Decimus - Rei soberano de Nimalia
- Elarion Vaelthor - Criador de Vezemir
- General Cassian Valerius
- Mestre Odran Veyl - Treinador de Varkh
- Mira Valen - Filha do chefe de Leth'valora
- Padre Oric - Sacerdote e escriba
- Theron Elensar

## Classes

### Classes oficiais (Old Dragon 2E)
- Clérigo
- Guerreiro
- Homem de Armas (legado)
- Ladrão
- Mago

### Classes opcionais
- Alquimista (suplemento independente)

## Facções

- Conclave dos Errantes
- Coroa de Nimalia
- Culto dos Sussurrantes
- Guarda Real de Nimalia
- Guardiões do Véu Cinzento
- Guilda dos Mercadores
- Sentinelas de Leth'valora

## Locais

- Capital de Nimalia
- Casa da Moeda de Nimalia
- Fortaleza de Gharok
- Leth'valora (vila élfica destruída)
- Maré Baixa
- Ruínas de Valthor

## Territórios

- Floresta de Avenor
- Bosque Sussurrante
- Campos de Earthropo
- Vale Dourado

## Lore

- Eclipse de Obsidiana
- Véu Cinzento
- Criadores
- Clã Sanguinallis

## Religiões

- Igreja das Chamas
- Fé dos Antigos
- Caminho dos Errantes (em desenvolvimento)

## Raças

- Humanos (população majoritária)
- Antropos (beastfolks de Nimalia)
- Elfos
- Kenku
- Anões (futuro reino)

## Capítulos da campanha

- EARTHROPO/EARTHROPO.md

## Documentos de workflow importantes

- Workflow/CANON.md - Cânone confirmado
- Workflow/RULES_SOURCES.md - Fontes de regras
- Workflow/LOCAL_ASSISTANT_PROTOCOL.md - Protocolo do assistente local
- Workflow/DEVIN_OPERATING_PROTOCOL.md - Protocolo de operação para IAs
- Workflow/CHARACTER_SCHEMA.md - Schema de personagens
- Workflow/IMAGE_ASSET_STANDARD.md - Padrão de imagens
- Workflow/GEOGRAPHY.md - Geografia
- Workflow/OUTLINES.md - Rascunhos de história
- Workflow/MIGRATION_LEDGER.md - Ledger de migração
